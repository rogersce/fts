...
#include<iostream>
#include"sim_tools/MersenneTwister.h"
#include<string>
#include<cstdlib>
#include<cstring>
...
namespace {
    ...
    void initRandom()
    {
        ...
        for(blitz::Array<double,3>::iterator it = mu.begin(); it != mu.end(); ++it)
        {
            *it = 2.0*(rng.rand53() - 0.5);
            total += *it;
        }
    ...
    int saveSilo(DBfile* db)
    {
        ...
        err += quadVar2Silo(db,mu.data(),Nx,Ny,Nz,"mu","Mesh");
        err += quadVar2Silo(db,dens.data(),Nx,Ny,Nz,"density","Mesh");
